...
**优先级最高：实机试玩打磨。** 本项目绝大多数改动只做过代码与数值层面验证。
开发环境的浏览器是隐藏视图（requestAnimationFrame 被暂停、截图不可用），
**画面与手感类问题无法由 agent 自行验证，只能靠人试玩反馈** —— 这是本项目最大的验证盲区。
历史上因此漏掉过：整套副武器/武器拾取的渲染代码从未写入 render.ts（编译通过但画面上什么都没有）、
商店台子渲染空指针导致进店黑屏。`npm run verify` 里的「实体渲染覆盖」检查就是为堵这个洞加的，改渲染后务必跑。
...
- 音效没有单独的近战/弹反音、拾取武器音。
- 无移动端适配（触屏没有输入路径）。
- nginx 的 `/game/` 段配了 `expires 1h`，`index.html` 也被缓存，部署后可能出现新旧资源错配，
  需要强刷。更好的做法是 HTML 不缓存、带哈希的静态资源长缓存（尚未改）。

### 已决策：暂不做登录/账号（2026-08-28）

讨论过三个方向，**结论是暂时不做**，先把游戏本体与手感打磨好：
- 方案 A 复用技能集市那台服务器（nginx/MySQL 现成），加 `/game-api/` 做云存档 + 排行榜，约一天工作量
- 方案 B 只做排行榜、不做账号（提交昵称成绩）
- 方案 C 暂不做 ← **当前选择**

理由：云存档与排行榜的价值前提是「游戏本身值得反复玩」，当前阶段手感问题的优先级更高。
真要做时首选方案 A。注意这会打破「零后端」前提，`save.ts` 目前是纯 localStorage、
未做远端存储抽象，届时需要先把读写收敛到一层接口后面。
